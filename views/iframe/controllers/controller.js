...
myApp.directive('myRepeatDirective', function() {
  return function(scope, element, attrs) {
    if (scope.$last){
      $('body').append('<script src="js/isotope.pkgd.min.js"></script><script src="js/flickity.pkgd.min.js"></script><script src="js/main.js"></script><script src="js/selectFx.js"></script>')
    }
  };
})


myApp.controller('AppCtrl', ['$scope', '$window', '$http', 
function($scope, $window, $http) {
	...
	$scope.addPost = function() {
		...
		$http.post('/api/posts', $scope.post).success(function(response){
			console.log("post id: " + $scope.post._id);
			refreshPost();
			//$scope.post = {url: parenturl}
		});
